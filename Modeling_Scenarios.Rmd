...
all_outputs_wmeta_scenarios = all_outputs_wmeta %>% 
  mutate(SeverityGroup = stringr::str_extract(ShortScenario, c("high"|"low"|"moderate")),
         AreaGroup = ) ### COME BACK HERE AND FIX 
...
all_outputs_wmeta %>% 
  ...
  filter(between(dates, as.POSIXct("2019-08-31"), as.POSIXct("2020-12-31"))) %>%
  na.omit() %>%
  ggplot(aes(x= dates, y=nit_mgL_fire, color=ShortScenario))+
  geom_point()+
  theme_classic()+
  labs(x = "date", y = "Nitrate mg/L")+
  theme(legend.position="right", 
