...
#### STEP 5 effect size #### 
Do the confidence intervals overlap with 0 or not. 
```{r}
rm(list=ls(all=T)) #this clears your Environment
...
### DOC ### 
```{r DOC}
mDOC <- lm(Effect_size ~ Koppen, data = effect_size_DOC)
...
dwplot(mDOC,
       vline = geom_vline(
         ...
         color = "grey",
         linetype = 2)) %>% 
  relabel_predictors(c(KoppenCsa = "Csa",
                       KoppenCsb = "Csb",
                       KoppenDfb = "Dfb",
                       KoppenDfc = "Dfc")) +
  ...
  geom_vline(xintercept = 0,
             color = "grey",
             linetype = 2) +
  ggtitle("Predicting Effect Size")
