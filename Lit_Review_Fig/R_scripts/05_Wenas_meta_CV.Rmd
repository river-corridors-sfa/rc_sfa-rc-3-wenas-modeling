...
ggplot(percent_difference_cv, aes(CVPD, Koppen, color = response_var),
  ...
  geom_pointrange(aes(xmin = CVPD - sdPD, xmax = CVPD + sdPD,
                      color = response_var),
                      position = position_dodge(width = -0.5), size = 1.5, data = percent_difference_cv) +
  ...
  theme_classic() +
  theme(axis.text.x = element_text(size = 30, angle = -30),
