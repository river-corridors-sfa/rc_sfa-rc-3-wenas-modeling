...
# Plot #
ggplot(analyte_table, aes(x = percent_change, y = response, fill = response)) +
...
# Plot without that huge nitrate outlier
analyte_table %>% 
  filter(percent_change > -300) %>% 
ggplot(aes(x = percent_change, y = response, fill = response)) +
  geom_vline(xintercept = 0, linewidth = 1.0, color = "red") +
  geom_boxplot() +
  xlab("Percent Change") +
  ylab("") +
...
ggsave("percent_change_no_outlier.pdf",
       path = ("~/GitHub/rc_sfa-rc-3-wenas-modeling/Lit_Review_Fig"),
       width = 10, height = 10, units = "in")
