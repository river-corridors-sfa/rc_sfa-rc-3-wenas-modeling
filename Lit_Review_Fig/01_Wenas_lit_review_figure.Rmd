...
metadata <- metadata[c("Study_ID", "Pair_JSC", "Climate", "Burn or Unburn", "Time_Since_Fire",
...
MeanConcTest <- metadata_long %>%
  group_by(Study_ID, Pair_JSC, Treatment, response_var, Climate) %>%
  dplyr::summarize(Ne = sum(Treatment == "Burn"),
                   Nc = sum(Treatment == "Unburn"),
                   Xe = mean(mean_concentration[Treatment == "Burn"], na.rm = TRUE),
                   Xc = mean(mean_concentration[Treatment == "Unburn"], na.rm = TRUE))
...
ggplot(analyte_table_filter, aes(x = Difference, y = response_var, color = Site, shape = response_var)) +
  geom_vline(xintercept = 0, linewidth = 1.0, color = "red") +
  geom_point() +
  xlab("Difference") +
  ylab("") +
...
ggplot(analyte_table_filter, aes(x = Difference, y = response_var, fill = response_var)) +
  geom_vline(xintercept = 0, linewidth = 1.0, color = "red") +
  geom_boxplot() +
  xlab("Difference") +
  ylab("") +
