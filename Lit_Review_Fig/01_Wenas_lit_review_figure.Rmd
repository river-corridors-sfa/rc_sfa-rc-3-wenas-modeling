...
mac.dir = "~/GitHub/rc_sfa-rc-3-wenas-modeling/Lit_Review_Fig"
...
# BRI advice for the below chunk #
...
metadata <- metadata %>% 
  mutate(metadata, TSF = ifelse(grepl("0-1|0-2|1 month|1|2|3|1-3|1-2", Time_Since_Fire), "0-3 years",
                              ifelse(grepl("4|5", Time_Since_Fire), "4-5 years",
                              ifelse(grepl("6|7|8|9|10", Time_Since_Fire), "6-10 years",
                              ifelse(grepl("11|12|13", Time_Since_Fire), ">10 years",
                              ifelse(grepl("Pre-fire", Time_Since_Fire), "Pre-Fire",
                              ifelse(grepl("1-13 years post fire", Time_Since_Fire), "Post-Fire", "Other")))))))
...
# with Pair as a test JSC #
MeanConcTest <- metadata_long %>%
  group_by(Study_ID, Pair_JSC, Treatment, response_var, Climate, TSF) %>%
  dplyr::summarize(Ne = sum(Treatment == "Burn"),
                   Nc = sum(Treatment == "Unburn"),
                   Xe = mean(mean_concentration[Treatment == "Burn"], na.rm = TRUE),
                   Sde = sd(mean_concentration[Treatment == "Burn"], na.rm = TRUE),
                   Xc = mean(mean_concentration[Treatment == "Unburn"], na.rm = TRUE),
                   Sdc = sd(mean_concentration[Treatment == "Unburn"], na.rm = TRUE))
...
analyte_table_test2[analyte_table_test2 == 0] <- NA

# study_ID 12 is the poo poo study with multiple controls

analyte_table_test3 <- analyte_table_test2 %>% 
  filter(Study_ID != "12") %>% 
  group_by(Study_ID, response_var, Climate, TSF) %>%
...
analyte_table_test3 <-analyte_table_test3 %>% 
  add_column(Xc123 = NA,
             Xc4 = NA,
             Xc5 = NA) %>% 
  ungroup()

analyte_table_test3_combine <- analyte_table_test3 %>% 
...
# pivoting to make the responses in one column
analyte_table_test3_long <- analyte_table_test3 %>% 
  pivot_longer(
...
analyte_table_filter <- analyte_table_test3_long %>% 
...
# pivoting to make the responses in one column
analyte_table_test4_long <- analyte_table_filter%>% 
  pivot_longer(
    cols = Xe1:Xc,
    names_to = "Stats",
    values_to = "Value",
...
df.summary.2 <- analyte_table_test4_long %>%
...
ggplot(analyte_table_filter, aes(Percent_Difference, Climate, color = response_var)) +
  ...
  geom_pointrange(aes(xmin = Percent_Difference-sd, xmax = Percent_Difference+sd, color = response_var), data = df.summary.2) +
  xlim(-100, 500) +
...
ggsave("site.difference.geom_pointrange.pdf",
       path = ("~/GitHub/rc_sfa-rc-3-wenas-modeling/Lit_Review_Fig"),
       width = 25, height = 20, units = "in")
