...
metadata <- metadata[c("Study_ID", "Pair_JSC", "Climate_JSC_from_climatecharts.com", 
                       "Burn or Unburn", "Time_Since_Fire_JSC",
...
metadata <- metadata %>% 
    ...
    rename("Climate" = "Climate_JSC_from_climatecharts.com")
...
MeanConcTest <- metadata_long %>%
  group_by(Study_ID, Pair_JSC, Treatment, response_var, Climate, Time_Since_Fire_JSC) %>%
  dplyr::summarize(Ne = sum(Treatment == "Burn"),
                   Nc = sum(Treatment == "Unburn"),
                   Xe = mean(mean_concentration[Treatment == "Burn"], na.rm = TRUE),
...
# analyte_table_test <- MeanConcTest %>%
#   select(-Treatment) %>%
#   group_by(Study_ID, Pair_JSC, response_var, Climate) %>%
#   summarize(Ne = sum(Ne),
#             Nc = sum(Nc),
...
analyte_table_test2 <- MeanConcTest %>%
  select(-Pair_JSC) %>%
  group_by(Study_ID, response_var, Climate, Time_Since_Fire_JSC) %>%
  summarize(Ne = sum(Ne),
            Nc = sum(Nc),
            Xe1 = sum(Xe[Pair_JSC == "Site 1"], na.rm = TRUE),
            Xe2 = sum(Xe[Pair_JSC == "Site 2"], na.rm = TRUE),
...
ggplot(analyte_table_filter, aes(x = Percent_Difference, y=Time_Since_Fire_JSC, fill = response_var)) +
  geom_boxplot() +
  geom_vline(xintercept = 0, linewidth = 1.0, color = "red") +
  xlab("Percent Difference") +
  ylab("") +
