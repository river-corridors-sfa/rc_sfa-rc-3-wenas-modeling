...
sev <- outputs_final %>% 
  filter(scenario_group == "Area Burned x Severity") %>% 
  group_by(percent, severity) %>% 
  summarize(nitrate_perc = mean(nitrate_perc, na.rm = T))

ggplot(sev, aes(x = percent, y = nitrate_perc, fill = severity)) + 
...
```{r Write out dataset}
write_csv(outputs_final, paste(Sys.Date(),"_output_datset_L1.csv", sep = ""))



#save(outputs_final, file = paste(Sys.Date(),"_output_datset_L1.csv", sep = ""))
```
